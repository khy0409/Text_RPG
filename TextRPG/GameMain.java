...
public class GameMain {
	...
	public static int runOneRound(Player2ndLevel[] attackParty, Monster[] defenseParty) {
		...
		// 스킬 사용 가능 상태 체크
		if(BuffTime%playerBuff  == 0 ) { //버프 쿨타임 주는 if문
			if(attackParty[randomPIdx] instanceof Healer) { //class Healer라면
				if(attackParty[randomPIdx].mp >= (attackParty[randomPIdx]).skillMp) { //mp가 skillMp보다 많거나 같다면
					for(int i=0; i<attackParty.length; i++) { //모든 player에게 heal
						 attackParty[randomPIdx].skill(attackParty[i]);
					 }
					 System.out.println("힐러가 전체 회복을 진행합니다");//print문 출력
				}else { //skillMp가 더 많다면
					attackParty[randomPIdx].healType(attackParty[randomPIdx]); //healType 부름(본인을 heal)
				}
			}else {//class healer가 아니라면
				if (attackParty[randomPIdx].mp >= (attackParty[randomPIdx]).skillMp) //mp가 skillMp보다 많거나 같다면
					attackParty[randomPIdx].buffSkill(defenseParty[target]); //버프 스킬 부름
				else {
					attackParty[randomPIdx].BuffAttackType(defenseParty[target]);//아니면 버프 어택 부름
				}
			}
			BuffTime++; //BuffTime++(한 턴 종료)
		}else{ //버프 주는 턴이 아니라면  //역시 디버킹~good~
			if(attackParty[randomPIdx] instanceof Healer) { //class Healer라면
				if(attackParty[randomPIdx].mp >= (attackParty[randomPIdx]).skillMp) {//mp가 skillMp보다 많거나 같다면
					 for(int i=0; i<attackParty.length; i++) { //모든 player에게 heal
						 attackParty[randomPIdx].skill(attackParty[i]);
					...
					attackParty[randomPIdx].healSay();
				}
			}else { //class healer가 아니라면
				if (attackParty[randomPIdx].mp >= (attackParty[randomPIdx]).skillMp)
					attackParty[randomPIdx].skill(defenseParty[target]); //common skill
				else
					attackParty[randomPIdx].attackType(defenseParty[target]);//common attack
			}
			BuffTime++; //BuffTime++(한 턴 종료)
		}
